...
#include <stdlib.h>
//stdlib is a library that allows for certain functions relating to memory. These libraries are probably the most common headers included in c and c++
#include <iostream>
...
int main()	//now, unlike java, C and C++ both start in a function called main not inside a class.
{		//you see that we declare main as an 'int'. I'll explain this later
	...
	cout << "a: " << a << "\nb: " << b << "\na+b: " << b + a << endl;
	//if you notice something, we can see that the << act like the + operator. It appends output
	//you can use \n for newline instead of endl if you wish. Endl stands for end line
	//for those that like the good old way of printing, you can use the stdio printf function
	printf("this came from printf\n");

	return 0;
	// it is standard that we return 0, indicating that we have run successfully. Otherwise we can return another status code, but don't worry about that for now.
